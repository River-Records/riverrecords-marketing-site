...
order: 2
description: "A clinical note is a bundle of facts grouped three ways at once — by time, by clinical thread, and by responsibility. Understanding those functions is the key to evaluating alternatives."
seoTitle: "What Is a Clinical Note? Time, Threads, and Attestation"
seoDescription: "A clinical note is a bundle of facts grouped three ways at once — by time, by clinical thread, and by responsibility. Understanding those functions is the key to evaluating alternatives."
relatedPosts:
  - slug: "part-2-siloed-medical-documentation-problems"
    title: "Siloed Documentation in a Collaborative World"
draft: false
---
...
![Figure 1.1. Under the current paradigm, a set of clinical facts makes up a note, and a set of notes (along with additional structured data) makes up a chart.](/images/book/fig-1-1-fact-note-chart.jpeg)

*Figure 1.1. Under the current paradigm, a set of clinical facts makes up a note, and a set of notes (along with additional structured data) makes up a chart.*
...
![Figure 1.2. The note as timeslice. Each tick represents a note in a patient's chart. Notes provide updates on a patient's status over a period of time. The length of that period depends on the nature of the note: a routine primary care note updates the patient's status after several months; during an inpatient admission, a new note updates the patient's status each day.](/images/book/fig-1-2-note-as-timeslice.jpeg)

*Figure 1.2. The note as timeslice. Each tick represents a note in a patient's chart. Notes provide updates on a patient's status over a period of time. The length of that period depends on the nature of the note: a routine primary care note updates the patient's status after several months; during an inpatient admission, a new note updates the patient's status each day.*
...
![Figure 1.3. The note as thread separator. A thread is a collection of notes with similar content or purpose; under the current paradigm each note belongs to a particular thread (indicated here by color). A patient's outpatient cardiology care is one thread; their inpatient treatment is another. Threads can overlap in time, as when inpatient care includes both physician and nursing threads.](/images/book/fig-1-3-note-as-thread.jpeg)
...
![Figure 1.4. The note as attestation bundle. When a clinician writes and signs a note, they attest to, and take responsibility for, each individual fact within it.](/images/book/fig-1-4-note-as-attestation.jpeg)

*Figure 1.4. The note as attestation bundle. When a clinician writes and signs a note, they attest to, and take responsibility for, each individual fact within it.*
